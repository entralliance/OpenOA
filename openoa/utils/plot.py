...
def plot_yaw_misalignment(
    ...
    # determine if confidence intervals should be plotted (i.e., UQ) based on dimension of data
    if (power_values_vane_ws.ndim == 2) & (curve_fit_params_ws.ndim == 2) & (yaw_misalignment_ws.ndim == 1):
        UQ = False
    elif (power_values_vane_ws.ndim == 3) & (curve_fit_params_ws.ndim == 3) & (yaw_misalignment_ws.ndim == 2):
        UQ = True
    else:
        raise ValueError(
    ...
    if len(ws_bins) == 1:
        ...
        fig, axs = plt.subplots(int(np.floor((len(ws_bins)-1)/3)+1), 3, sharex=True, **figure_kwargs)
        N_col = 3
    elif len(ws_bins) >= 10:
    ...
    for i, ws in enumerate(ws_bins):
        ...
        if UQ:
            ...
            axs[int(np.floor(i/N_col))][i % N_col].plot(
                vane_bins,
                cos_curve(vane_bins,curve_fit_params_ws[:,i,0].mean()/norm_factor,curve_fit_params_ws[:,i,1].mean(),curve_fit_params_ws[:,i,2].mean()),
                color=curve_fit_color_code,
                label="_nolabel_"
            )
            axs[int(np.floor(i/N_col))][i % N_col].plot(
                2*[curve_fit_params_ws[:,i,1].mean()],
                [0.01*np.floor(y_min/norm_factor/0.01), 0.01*np.ceil(y_max/norm_factor/0.01)],
                color=curve_fit_color_code,
                linestyle="--",
            ...
            axs[int(np.floor(i/N_col))][i % N_col].scatter(
                vane_bins,
                power_values_vane_ws[i,:]/norm_factor,
                color=power_color_code
            )
            axs[int(np.floor(i/N_col))][i % N_col].plot(
                vane_bins,
                cos_curve(vane_bins,curve_fit_params_ws[i,0]/norm_factor,curve_fit_params_ws[i,1],curve_fit_params_ws[i,2]),
                color=curve_fit_color_code,
                label="_nolabel_"
            )
            axs[int(np.floor(i/N_col))][i % N_col].plot(
        ...
        if ylim != (None, None):
            ...
            axs[int(np.floor(i/N_col))][i % N_col].set_ylim((0.1*np.floor(y_min/norm_factor/0.1), 0.1*np.ceil(y_max/norm_factor/0.1)))
    ...
    if UQ:
        ...
        fig.suptitle(f"Turbine {turbine_id}, Yaw Misalignment = {np.round(mean_yaw_misalignment,1)}$^\circ$ [{np.round(yaw_misalignment_95CI[0],1)}$^\circ$, {np.round(yaw_misalignment_95CI[1],1)}$^\circ$]")
    else:
        fig.suptitle(f"Turbine {turbine_id}, Mean Yaw Misalignment = {str(np.round(mean_yaw_misalignment,1))}$^\circ$")
